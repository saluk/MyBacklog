...
def path_to_icon(game, filecache_root, category="icon", url=""):
    ...
    if url:
        return (
            filecache_root + "/cache/icons/" + urlclean(url) + ".png",
            "download",
            url,
        )
    elif game.get_exe():
        ...
        return (
            filecache_root + "/cache/icons/" + urlclean(exe_path) + ".png",
            "extract",
            exe_path,
        )
    elif game.get_gba():
        ...
        return (
            filecache_root + "/cache/icons/" + urlclean(gba_path) + ".png",
            "gba",
            gba_path,
        )
    else:
...
def icon_in_cache(
    game, size, cache, filecache_root, category="icon", imode="qt", url=""
):
...
def icon_for_game(
    game, size, icon_cache, filecache_root, category="icon", imode="qt", url=""
):
    ...
    if os.path.exists(fpath) and not (fpath, size) in icon_cache:
        if imode == "path":
            ...
            if not qp.isNull():
                ...
                qp = qp.scaled(QSize(size, size), mode, Qt.SmoothTransformation)
            else:
