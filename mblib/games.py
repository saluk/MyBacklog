...
class Game:
    ...
    def generate_gameid(self):
        ...
        if self.package_data:
            ...
            if self.package_data["type"] == "bundle":
                pool.update(
                    bytes(self.package_data["source_info"]["package_source"], "utf8")
                )
                pool.update(
                    bytes(str(self.package_data["source_info"]["package_id"]), "utf8")
                )
                ...
                try:
                    pool.update(
                        bytes(
                            str(self.package_data["source_info"]["id_within_package"]),
                            "utf8",
                        )
                    )
                except:
        ...
        for g in games.games.values():
            ...
            for ps in g.sources:
                for gs in self.sources:
                    ...
                    if (
                        ps["source"] == "humble"
                        and gs["source"] == "humble"
                        and ps["package"] == gs["package"]
                    ):
                        add = True
            if add:
                gamelist.append(g)
    ...
    def source_match(self):
        string = ""
        print(self.sources)
        for s in sorted(self.sources, key=lambda ob: ob["source"]):
        ...
        def get_source_info(source):
            if source["source"] == "gog":
                return {
                    ...
                    "id_within_package": self.name_stripped,
                }
    ...
    def get_path(self):
        ...
        if not self.games:
            return ""
        files = (
            self.games.local.get("game_data", {}).get(self.gameid, {}).get("files", [])
        )
        for f in files:
            if f["primary"]:
                return f["path"]
    ...
    def update_dynamic_fields(self):
        if (
            not self.website
            and self.sources
            and get_source(self.sources[0]["source"]).generate_website
        ):
            self.website = get_source(self.sources[0]["source"]).generate_website(
                self, self.sources[0]
            )
...
class Games:
    ...
    def update_game(self, gameid, game):
        ...
        if game.finished:
            cur_game.finished = 1
        if game.lastplayed and (
            not cur_game.lastplayed or stot(game.lastplayed) > stot(cur_game.lastplayed)
        ):
        ...
        if diff:
            ...
            print(
                "update 2",
                cur_game.gameid,
                cur_game.source_match,
                ">",
                game.name.encode("utf8"),
                game.source_match,
            )
            self.log.write(
                "GAMEDB: Update ", previous_data["gameid"], " ", nicediff(diff)
            )
        else:
