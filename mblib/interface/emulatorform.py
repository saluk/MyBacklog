...
class EmulatorForm(QWidget):
    def __init__(self, app, message="", highlight_fields=[], dock=False):
        ...
        for emu in sorted(sources.all.keys()):
            if sources.all[emu].__class__.__name__ != "EmulatorSource":
                continue
            self.fields[emu] = copy.deepcopy(
                self.app.games.local["emulators"].get(emu, {})
            )
    ...
    def save_close(self):
        for emu in self.fields:
            args = [
                x
                for x in [self.fields[emu]["w1"].text()]
                + self.fields[emu]["w2"].text().split(" ")
                if x.strip()
            ]
            self.app.games.local["emulators"][emu] = {
                "args": args,
                "rom_extension": self.fields[emu]["w3"].text(),
            }
        self.app.save()
        self.delete()
