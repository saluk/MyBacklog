    ...
    def __init__(self, game, app, edit_widget):
        ...
        for image_type in ["logo"]:
            ...
            icon = icons.icon_for_game(
                self.game, 64, self.app.gicons, self.app.config["root"], image_type
            )
            if icon:
    ...
    def init(self, game):
        ...
        for i2, source in enumerate(game.sources):
            ...
            if source["source"] in self.app.icons:
                icon.setPixmap(
                    self.app.icons[source["source"]].scaled(48, 48, Qt.KeepAspectRatio)
                )
            layout.addWidget(icon, i, 1)
            source_layout = QGridLayout()
            for i3, prop in enumerate(games.get_source(source["source"]).source_args):
                self.addwidget(
                    i3, source, prop, source_layout, mode="source", source_index=i2
                )
    ...
    def addwidget(self, i, resource, prop, layout, mode="game", source_index=None):
        ...
        if prop == "notes":
            ...
            button.clicked.connect(
                make_callback(self.expand_notes, resource, prop, edit, i, layout)
            )
...
class GameOptions(QWidget):
    def __init__(self, game, app, new=False):
        ...
        icon = icons.icon_for_game(
            game, 180, self.app.gicons, app.config["root"], "logo"
        )
        if icon:
            iconw = QLabel()
        ...
        if game.is_installed():
            ...
            play_options.append(
                ("Launch", make_callback(self.app.run_game_notimer, game))
            )
        play_options.append(
        ...
        if game.hidden:
            ...
            w.clicked.connect(
                make_callback(self.app.operation, "set", game, "hidden", 0)
            )
            buttons.addWidget(w)
        else:
            w = QPushButton("Hide")
