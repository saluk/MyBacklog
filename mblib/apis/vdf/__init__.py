...
def parse(fp, mapper=dict, merge_duplicate_keys=True, key_lower=True):
    """
    ...
    for idx, line in enumerate(fp):
        ...
        if expect_bracket:
            raise SyntaxError(
                "vdf.parse: expected openning bracket (line %d)" % (idx + 1)
            )
        ...
        if line[0] == "}":
            ...
            raise SyntaxError(
                "vdf.parse: one too many closing parenthasis (line %d)" % (idx + 1)
            )
            ...
            # we've matched a simple keyvalue pair, map it to the last dict obj in the stack
                ...
                if match.group("vq_end") is None and match.group("qval") is not None:
                    try:
                        ...
                        raise SyntaxError(
                            "vdf.parse: unexpected EOF (open value quote?)"
                        )
...
def binary_loads(s, mapper=dict, merge_duplicate_keys=True, alt_format=False):
    ...
    if len(s) != idx or len(stack) != 1:
        raise SyntaxError(
            "Binary VDF ended at index %d, but length is %d" % (idx, len(s))
        )
