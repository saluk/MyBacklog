...
def login_for_chat():
    ...
    sess.headers[
        "User-Agent"
    ] = "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/31.0.1650"
...
def set_username(sess, name):
    ...
    r = sess.post(
        "http://steamcommunity.com/profiles/76561197999655940/edit", data=data
    )
...
def scrape_app_page(appid, cache_root="", logger=None):
    ...
    cache_url = (
        cache_root
        + "/cache/steamapi/"
        + url.replace(":", "").replace("/", "").replace("?", "QU").replace("&", "AN")
    )
    dat = {}
    if not os.path.exists(cache_url):
        ...
        if category_html:
            categories = [
                x.text.strip() for x in category_html.find_all("a") if x.text.strip()
            ]
        ...
        for block in soup.find_all(class_="block"):
            if block.find_all(class_="vrsupport"):
                vr = [
                    x.text.strip().replace(" ", "_")
                    for x in block.find_all("a")
                    if x.text.strip()
                ]
...
def import_steam(
    apikey=MY_API_KEY, userid=MY_STEAM_ID, cache_root=".", user_data=None, logger=None
):
    ...
    if user_data:
        print(
            "loading",
            user_data["userlocalconfigstore"]["software"]["valve"]["steam"]["apps"],
        )
    ...
    for g in get_games(apikey, userid):
        ...
        if "img_icon_url" in g and g["img_icon_url"].strip():
            icon_url = (
                "http://media.steampowered.com/steamcommunity/public/images/apps/%(appid)s/%(img_icon_url)s.jpg"
                % g
            )
        if "img_logo_url" in g and g["img_logo_url"].strip():
            logo_url = (
                "http://media.steampowered.com/steamcommunity/public/images/apps/%(appid)s/%(img_logo_url)s.jpg"
                % g
            )
        lastplayed = None
        ...
        game = games.Game(
            ...
            sources=[{"source": "steam", "id": str(g["appid"])}],
...
def load_userdata(path=""):
    ...
    if (
        path in load_userdata.cache
        and (time.time() - load_userdata.cache["_time_"]) < 10
    ):
...
def find_steam_files():
    ...
    for path in systems.steam_paths:
        ...
        for user_id in os.listdir(p2):
            ...
            users[username] = {
                "local": lc,
                "shortcut": p3 + "/shortcuts.vdf",
                "account_id": int(user_id) + 76561197960265728,
            }
    return users
...
class Steam:
    ...
    def export(self):
        ...
        create_nonsteam_shortcuts(
            self.app.games.list(None), self.shortcut_folder, self.app.config["root"]
        )
    ...
    def running_game_id(self):
        ...
        print(url)
        r = requests.get(
            USER_DATA_URL % {"apikey": self.api_key, "steamid": self.user_id}
        )
        data = r.json()
        print(data)
...
def shortcut_id(
    name="Assassin's Creed Syndicate(mbl)",
    target='"C:\\Program Files (x86)\\MyBacklog\\MyBacklog.exe" --play b998da00d9917a242109d9069c08e1a6',
):
    """
...
def make_grid_for_shortcut(name, target, iconpath, shortcutpath):
    ...
    shutil.copy(
        iconpath, os.path.join(os.path.dirname(shortcutpath), "grid", gameid + ".png")
    )
...
def create_nonsteam_shortcuts(gamelist, shortcutpath, filecache_root=""):
    ...
    for game in gamelist:
        ...
        if not game.get_source("steam"):
            ...
            if "dragon" in game.name.lower():
                print(
                    game.get_source("steam"),
                    "(skip)Game is in steam or no install path",
                )
            continue
        tags = ["mybacklog"]
        if game.finished:
        ...
        iconpath = icons.icon_for_game(
            game, 64, {}, filecache_root, category="icon", imode="path"
        )
        name = game.name_ascii + "(mbl)"
        exe = (
            '"C:\\Program Files (x86)\\MyBacklog\\MyBacklog.exe" --play %s'
            % game.gameid
        )
