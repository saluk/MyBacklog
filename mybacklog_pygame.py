    ...
    if os.path.exists(root["accounts"]):
        try:
            saved_accounts = json.loads(
                crypter.read(open(root["accounts"]).read(), "{}")
            )
        except:
            raise
            saved_accounts = {}
...
def set_accounts(account, app):
    ...
    humble = humbleapi.Humble(
        app, account["humble"]["username"], account["humble"]["password"]
    )
...
def get_text(game, selected):
    if (game.name, selected) not in text_cache:
        text = (
            game.name + " ... " + game.hours_played + " last:" + game.last_played_nice
        )
        color = [250, 250, 250]
        if selected:
            color = [250, 150, 150]
...
class MyBacklogApp:
    ...
    def draw(self):
        ...
        if self.running_game:
            screen.blit(
                font.render(
                    "Playing " + self.running_game.name + " [enter to stop]",
                    [250, 150, 150],
                )[0],
                [5, 5],
            )
